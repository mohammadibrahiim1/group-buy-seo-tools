  ...
  return (
    <div>
      <div className="indicator" onClick={() => setOpenToggler(!openToggler)}>
        <span className="indicator-item badge badge-warning">{cart.length}</span>
      ...
      <div className={`cart-item bg-base-100  ${openToggler ? "cart-item-open" : "cart-item-closed"}`}>
        <div className="text-end" onClick={() => setOpenToggler(!openToggler)}>
          <button className="btn btn-sm"> {openToggler ? <MdClose className="h-5 w-5 " /> : ""}</button>
        </div>
        <div className="text-center">
          <p className="text-sm text-error">all in one package($250) 50$/30days</p>
